...
eigencharges = readdata("eigencharges")
eigenpseudolambdas = np.diag(np.loadtxt("eigenpseudolambdas.csv",delimiter=","))
pfacepos = readdata("pfacepos")
pfacearea = readdata("pfacearea")
...
#%% S P E C T R U M _ superfocused
...
plot0 = ax0.pcolormesh(X*1e9,Y*1e9, gammas[0],
                       cmap = 'inferno',
                       )

plot1 = ax1.pcolormesh(X*1e9,Y*1e9, gammas[1],
                       cmap = 'inferno',
                       )

ax0.set_title(f'Bonding {evenergies[0]:.3} eV')
ax1.set_title(f'Antibonding {evenergies[1]:.3} eV')

colorbar0 = fig.colorbar(plot0, ax = ax0 )
colorbar1 = fig.colorbar(plot1, ax = ax1 )

colorbar0.mappable.set_clim(*colorbar1.mappable.get_clim())
...
np.savetxt(f"map_bonding_li{li}_lf{lf}_{meshx}x{meshy}.csv", bonding, header=str(metadata))

metadata['bondingantibonding'] = 1
metadata['omega'] = omegas[metadata['bondingantibonding']]
np.savetxt(f"map_antibonding_li{li}_lf{lf}_{meshx}x{meshy}.csv", antibonding, header=str(metadata))
...
#%% DICHROISM
dataB1, metadataB1 = readdata('map_bonding_li1_lf1_64x64.csv')
dataAB1, metadataAB1 = readdata('map_antibonding_li1_lf1_64x64.csv')
dataB2, metadataB2 = readdata('map_bonding_li-1_lf-1_64x64.csv')
dataAB2, metadataAB2 = readdata('map_antibonding_li-1_lf-1_64x64.csv')
...
for i in range(7):
    ...
    data1, metadata1 = readdata(fname1)
    omegas1 = data1[:,0]
    gammas1 = data1[:,1]
    
    fname2 = f"fullspectrum_li-1_lf-1_xc{xc}_yc{xc}.csv"
    data2, metadata2 = readdata(fname2)
    omegas2 = data2[:,0]
    gammas2 = data2[:,1]
