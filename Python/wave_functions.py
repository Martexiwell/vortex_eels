...
"""
Now, we define our function according to the expression above. The integral ```integrate.quad``` from scipy takes as an argument only a float(?), 
not an array so we had to use for cycle. By deleting/inserting the ```#``` symbol in the return statement the function can be simply modified to 
return only the array of values or tuple of an array of values and an array of estimated absolute errors from the integration.
"""

def psi_perp_radial(l, r, aperture_q):
    """
    Calculates the radial profile of bessel beam
    """
